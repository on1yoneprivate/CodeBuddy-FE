...
const PlanPage = () => {
  ...
  useEffect(() => {
    ...
    // 상태 업데이트를 비동기적으로 처리하고 다른 작업들과 병렬로 실행
    (async () => {
      ...
      await fetchChatDetails(); // 데이터를 가져오는 함수 호출
    })();
  ...
  const handleNewMessage = async (message: Message[]) => {
    ...
    try {
      const apiUrl = `/main/ask/${chatroomId}?categoryType=PLAN`;
      ...
      if (!chatroomId || isNaN(parseInt(chatroomId))) {
      ...
      const responseMessage: Message = {
        ...
        output: formattedContent, // 포맷팅된 내용을 사용
      };
  
      // 상태 업데이트를 비동기적으로 처리, 질문 목록에 새로운 메시지 추가
      setQuestions(prevQuestions => [...prevQuestions, responseMessage]);
      localStorage.setItem(`chatroom-${chatroomId}`, JSON.stringify({ questions: [...questions, responseMessage] }));
