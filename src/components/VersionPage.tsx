...
const VersionPage: React.FC<VersionProps> = ({ questionTitles, questions: initialQuestions, fetchQuestionTitles, category }) => {
  ...
  const fetchInitialData = async (userId: string, categoryType: string) => {
    try {
      const [chatroomResponse, savedDataResponse] = await Promise.all([
        ...
        fetchWithToken(`/main/version/saved?userId=${userId}`, {
          method: 'GET',
        }),
      ]);
