...
const CodePage: React.FC<CodeProps> = ({ questionTitles, questions: initialQuestions, fetchQuestionTitles, category, chatroomId }) => {
  ...
  const fetchInitialData = async (userId: string, categoryType: string) => {
    try {
      const [chatroomResponse, savedDataResponse] = await Promise.all([
        ...
        apiClient.get('/main/code/saved', { params: { userId } }),
      ]);
  ...
  const handleSaveToSidebar = (title: string) => {
    ...
    const newSidebarItem = { chatroomId: currentChatroomId, title, category: 'code' };
