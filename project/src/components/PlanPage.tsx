...
const PlanPage: React.FC<PlanProps> = ({ questionTitles, questions: initialQuestions, fetchQuestionTitles, category, chatroomId }) => {
  ...
  const fetchInitialData = async (userId: string, categoryType: string) => {
    try {
      const [chatroomResponse, savedDataResponse] = await Promise.all([
        ...
        apiClient.get('/main/plan/saved', { params: { userId } }),
      ]);
  ...
  const handleSaveToSidebar = (title: string) => {
    ...
    const newSidebarItem = { chatroomId: currentChatroomId, title, category: 'plan' };
