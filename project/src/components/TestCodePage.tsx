...
const TestCodePage: React.FC<TestCodeProps> = ({ questionTitles, questions: initialQuestions, fetchQuestionTitles, category, chatroomId }) => {
  ...
  const fetchInitialData = async (userId: string, categoryType: string) => {
    try {
      const [chatroomResponse, savedDataResponse] = await Promise.all([
        ...
        apiClient.get('/main/testcode/saved', { params: { userId } }),
      ]);
