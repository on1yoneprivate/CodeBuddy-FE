...
const VersionPageContainer = styled.div`
  display: flex;
`;
...
const VersionPage: React.FC<VersionProps> = ({ questionTitles, questions: initialQuestions, fetchQuestionTitles, category, chatroomId }) => {
  ...
  const fetchInitialData = async (userId: string, categoryType: string) => {
    try {
      const [chatroomResponse, savedDataResponse] = await Promise.all([
        ...
        apiClient.get('/main/version/saved', { params: { userId } }),
      ]);
  ...
  const handleSaveToSidebar = (title: string) => {
    ...
    const newSidebarItem = { chatroomId: currentChatroomId, title, category: 'version' };
  ...
  const handleNewMessage = async (message: Message[]) => {
    ...
    // API 사용
    try {
      await apiClient.put(`save/${chatroomId}`, {questions});
      console.log('데이터 저장 성공(save/chatroomId) ', newChatroomId);
    } catch (error) {
